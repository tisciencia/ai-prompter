...
async def test_limited_endpoint():
    ...
    assert response.status_code == 200
    assert response.json() == {"message": "Esse endpoint é limitado a 5 requisições por minuto."}
...
async def test_secure_info_endpoint():
    ...
    assert response.status_code == 200
    assert response.json() == {"message": "Você acessou um endpoint protegido com API Key."}
